...
void binary(Mytree *myt,char (*strings)[100],char (*color)[100]){
    int *b;
	int n2=100;
...
int forets(Mytree *myt,char *a,int n,char (*strings)[100],char (*color)[100]){
    int i=1,j=0;
...
void recompose_p(Mytree *myt,char *a,int k,int n,int m,char (*strings)[100],char (*color)[100]){
    int i,j,valide=0;
    for(k=1;k<=n;k++){
            a[(2*k)-1]='(';
            a[2*k]=')';
